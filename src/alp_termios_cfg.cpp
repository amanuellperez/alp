...
void Termios_cfg::print_cflag(std::ostream& out) const 
{
    ...
    pflag(out, cfg_.c_cflag & PARODD, "If  set, then parity for input and output is odd; otherwise even parity is used");
    pflag(out, cfg_.c_cflag & HUPCL, "Lower modem control lines after last process closes  the	device (hang up)");
...
void Termios_cfg::print_lflag(std::ostream& out) const 
{
    ...
    pflag(out, cfg_.c_lflag & ISIG, "When any of the characters INTR, QUIT, SUSP, or DSUSP are received, generate the corresponding signal");
    ...
    pflag(out, cfg_.c_lflag & NOFLSH, "Disable flushing the input and output queues when generating signals for the INT, QUIT, and SUSP characters");
    pflag(out, cfg_.c_lflag & TOSTOP, "Send the SIGTTOU signal to the process group of a background process which tries to write to its controlling terminal");
